...
using Antmicro.Renode.Core.Structure;   // IPeripheralContainer<ICFU, NumberRegistrationPoint<int>>
using Antmicro.Renode.Exceptions; // public void Register()
...
using Antmicro.Renode.Peripherals.CFU; // ~/park/renode/renode/src/Infrastructure/src/Emulator/Main/Peripherals/CFU 
                                                                                                            //   |__ ICFU.cs
                                                                                                            //   |__ PimCfu.cs
...
// {
//     //
//     public class SpringbokCFU : ICFU
//     {
//         //
...
//         public uint GetCFUMacOutValue()
//         {
//             this.Log(LogLevel.Info, $"[CFU] GetMacOutValue called. Current macOutValue: {macOutValue}");
//             return macOutValue;
//         }

//         public void SetCFUMacOutValue(uint value)
//         {
//             this.Log(LogLevel.Info, $"[CFU] Setting macOutValue to {value}");
//             macOutValue = value;
//         }

//         public void Reset()
//         {
//             macOutValue = 0; // Reset macOutValue to a default value <- necessary
//         }
...
{
    public class SpringbokRiscV32 : RiscV32 //, IPeripheralContainer<ICFU, NumberRegistrationPoint<int>>
    {
        // TODO:
        // private SpringbokCFU pim_cfu; // Set PIM object  
        // private bool cfuRegistered = false;
        ...
        public SpringbokRiscV32(Core.Machine machine,
                                ...
                                string cpuType = "rv32imfcv_zicsr")
//            : base(null, cpuType, machine, hartId, privilegeArchitecture, endianness)
            : base(machine, cpuType, null, hartId, privilegeArchitecture, endianness)
        {
            // TODO:
            // Initialize the CFU
            // pim_cfu = new SpringbokCFU();
            // // cfu.ConnectedCpu = this;
            // RegisterCFU(pim_cfu); // Register the CFU
            // TestCFU();
            //
        ...
        // public void TestCFU()
        // {
        //     this.Log(LogLevel.Info, "Testing CFU manually");

        //     pim_cfu.SetCFUMacOutValue(0x12345678); // decimal 305419896
        //     var value = pim_cfu.GetCFUMacOutValue();

        //     this.Log(LogLevel.Info, $"Test CFU MacOutValue: {value}\n"); // Should log 0x12345678, 'd305419896
        // }
        ...
        public override void Reset()
        {
            ...
            // Reset the CFU
            // if (cfuRegistered) // if (pim_cfu != null)
            // {
            //     pim_cfu.Reset();
            // }
        ...
        // public void RegisterCFU(SpringbokCFU pim_cfu)
        // {
        //     if (cfuRegistered)
        //     {
        //         throw new InvalidOperationException("CFU is already registered.");
        //     }

        //     this.pim_cfu = pim_cfu;
        //     pim_cfu.ConnectedCpu = this;
        //     cfuRegistered = true;
        // }

        // TODO:
        // private SpringbokRiscV32_PimCfu PimCfu;
        private SpringbokRiscV32_ControlBlock ControlBlock;
        private bool ControlBlockRegistered = false;

        private int custom3CallCount = 0;
        private void HandleSpringbokCustom3(UInt64 opcode)
        {
            ...
            // this.Log(LogLevel.Error, $"[ControlBlock -> START] rd: {rd}, rs1: {rs1}, rs2: {rs2}");
            // this.Log(LogLevel.Error, $"[HandleSpringbokCustom3 Call Count] {custom3CallCount} times called");
            ...
            switch(funct3)
            {
                ...
                ///// Read mac_out 
                case 4:
                    var macOutValue = ControlBlock.GetMacOutValue();
                    // this.Log(LogLevel.Info, $"[ControlBlock] Reading MacOut: {macOutValue}");
                    // this.Log(LogLevel.Info, $"rd: {rd}, rs1: {rs1}, rs2: {rs2}");
                    X[rd] = macOutValue;
                    break;

                ///// CFU mac_out
                // case 5:
                //     this.Log(LogLevel.Info, $"hello CFU world !");
                //     this.Log(LogLevel.Info, $"CASE5 >>> rd: {rd}, rs1: {rs1}, rs2: {rs2}");

                //     // Get the macOutValue from the CFU
                //     var cfuMacOutValue = pim_cfu.GetCFUMacOutValue();
                //     this.Log(LogLevel.Info, $"CFU MacOutValue: {cfuMacOutValue}");
                //     X[rd] = cfuMacOutValue;
                //     break;
        ...
        {
            // validate only privilege level when accessing CSRsG
        ...
        IKnownSize
    {
        ...
        {
            this.Log(LogLevel.Info, $"GetMacOutValue called. Current macOutValue: {macOutValue}");
            
            this.Log(LogLevel.Info, $"MAC_OUT value read: {macOutValue}");
            return macOutValue;
        }
        ...
        {
            // MacOut = value;
            this.Log(LogLevel.Info, $"Setting MacOut to {value}");
            macOutValue = value;
        }
            ...
            Registers.Control.Define32(this, resetValue: 0x00000003)
                    .WithValueField(0, 19, name: "FREEZE_VC_RESET_PC_START", writeCallback: (_, val) =>
                    {
                        ...
                        if (((mode & Mode.SwReset) != 0) && ((newMode & Mode.SwReset) == 0))
                        {
                            ...
                            // Ã¬Æ’Ë†Ã«Â¡Å“Ã¬Å¡Â´ Ã«Â³â‚¬Ã¬Ë†Ëœ Ã¬ï¿½Â´Ã«Â¦â€žÃ¬ï¿½â€ž Ã¬â€šÂ¬Ã¬Å¡Â©
                            ulong startAddress = (val & ~(ulong)Mode.Mask) + Machine.SystemBus.GetRegistrationPoints(IMem).First().Range.StartAddress;
